...
async function main() {
	...
	const totalLines = lines.filter(l => parseYamlListVideoLine(l) && !isCommentLine(l)).length
	...
	for (const line of lines) {
		...
		process.stdout.write(`${progress} Checking: ${parsed.title || parsed.url} ... `)
		...
		process.stdout.write(`${progress} Searching replacement for: ${parsed.title || '(no title)'} ... `)
		const replacement = await findReplacement(parsed)
		if (replacement) {
			process.stdout.write(`FOUND (score=${replacement.score.toFixed(2)}) -> ${replacement.url}\n`)
...
async function validateYouTubeUrlWithOptions(url, options) {
	...
	const needsMetadata = options && (options.includePublishDate || options.includeViewCount || options.includeLikes)
	const metadata = needsMetadata ? await getWatchMetadata(id, watchUrl) : { publishDate: null, viewCount: null, likeCount: null }
	const publishDate = options && options.includePublishDate ? metadata.publishDate : null
	const viewCount = options && options.includeViewCount ? metadata.viewCount : null
...
async function fetchWatchMetadata(watchUrl) {
	...
	if (!response.ok) return { publishDate: null, viewCount: null, likeCount: null }

	const html = await response.text()
	if (!html) return { publishDate: null, viewCount: null, likeCount: null }

	const publishDate = extractPublishDateFromLdJson(html) || extractPublishDateFromMeta(html) || null
	const viewCount = extractViewCountFromLdJson(html) ?? extractViewCountFromVideoDetails(html) ?? null
	const likeCount = extractLikesFromLdJson(html) ?? extractLikesFromVideoDetails(html) ?? null
...
async function findReplacement(entry) {
	...
	const searchResults = await mapWithConcurrency(queries, SEARCH_QUERY_CONCURRENCY, q => searchYouTubeCandidates(q))
	...
	const validations = await mapWithConcurrency(uniqueCandidates, VALIDATION_CONCURRENCY, async candidate => {
	...
	const scored = await mapWithConcurrency(topCandidates, METADATA_CONCURRENCY, async c => {
...
function buildSearchQueries(entry) {
	...
	const parts = title.split(' - ').map(s => s && s.trim()).filter(Boolean)
...
async function searchYouTubeCandidates(query) {
	...
	const html = await response.text()
	const ids = extractVideoIdsFromSearchHtml(html).slice(0, SEARCH_CANDIDATE_LIMIT)
...
function similarityScore(original, candidate, query, ageScoreInput, viewsScoreInput, likesScoreInput) {
	if (!original || !candidate) return 0
	...
	return Math.min(1, 0.65 * textScore + 0.1 * likesScore + 0.1 * ageScore + 0.15 * viewsScore)
}
...
function computeAgeScore(publishDate) {
	...
	const now = new Date()
	const years = (now.getTime() - published.getTime()) / (1000 * 60 * 60 * 24 * 365.25)
	if (years <= 0) return 0
...
function computeViewsScore(viewCount) {
	if (typeof viewCount !== 'number' || Number.isNaN(viewCount) || viewCount <= 0) return 0
...
function computeLikesScore(likeCount) {
	if (typeof likeCount !== 'number' || Number.isNaN(likeCount) || likeCount <= 0) return 0
...
function extractPublishDateFromMeta(html) {
	const m = html.match(/itemprop="datePublished"\s+content="(\d{4}-\d{2}-\d{2})"/)
	return m ? m[1] : null
}
...
function extractLikesFromLdJson(html) {
	const m = html.match(/"aggregateRating"\s*:\s*{[^}]*"ratingCount"\s*:\s*"(\d+)"/)  
	if (!m) return null
	const parsed = Number(m[1])
	return Number.isFinite(parsed) ? parsed : null
}
...
function shouldRetryStatus(status) {
	return status === 429 || status === 500 || status === 502 || status === 503 || status === 504 || status === 599
}
...
async function fetchWithTimeout(url) {
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		...
		try {
			...
			if (!shouldRetryStatus(response.status) || attempt === maxAttempts) return response
		} catch {
			if (attempt === maxAttempts) return { ok: false, status: 599, text: async () => '' }
		} finally {
